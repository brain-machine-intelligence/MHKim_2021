...
class _Format(object):
    ...
    def pack(self, key, *args):
        ...
        result = struct.pack(bytes(fmt, "utf-8"), bytes(key, "utf-8"), *args)
        return result
...
def pstring(s):
    ...
    ps = struct.pack(bytes(fmt, "utf-8"), bytes(s, "utf-8"))
    # result = struct.pack(bytes(fmt, "utf-8"), bytes(key, "utf-8"), *args)
...
class _DataFormat(object):
    ...
    def _make_event_header(self, size_of_the_rest, timestamp, duration, keycode):
        ...
        result_str = struct.pack(bytes("=sH2L4s", "utf-8"), bytes('D', "utf-8"), total_length, timestamp, duration, keycode)
...
class Netstation(object):
    ...
    def GetServerResponse(self, b_raise=True):
        ...
        """
        code = self._socket.read(1).decode()
    ...
    def send_event(self, key, timestamp=None, label=None, description=None, table=None, pad=False):
        ...
        message = self._data_fmt.pack(bytes(key, "utf-8"), timestamp, label, description, table, pad)
        self._socket.write(message)
