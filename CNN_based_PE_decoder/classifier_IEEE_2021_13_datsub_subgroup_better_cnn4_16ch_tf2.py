...
class networks():
    def __init__(self, ep):
        ...
        # 0.00003 was good
        ####### 아래 애
        self.c_lr = c_learning_rate = 3e-7  # 1e-8
        # training_epoch = 10
        self.training_epoch = training_epoch = 3000

        ##### 얘 조절하면
        self.c_training_epoch = c_training_epoch = 3000  # 2000
    ...
    def init_net(self):
        ...
        self.bn1 = tf.layers.batch_normalization(conv1)
        ...
        self.pool4 = tf.nn.max_pool(self.conv4_out, ksize=[1, 2, 2, 1], strides=[1, 1, 1, 1], padding='SAME')

        # gap = tf.reduce_mean(tf.reshape(pool1, shape=(-1,80*27,1)), axis=1)
        self.gap = tf.reduce_mean(tf.reshape(self.pool4, shape=(-1,11*31,64)), axis=2)

        self.weight_ = tf.Variable(tf.zeros((11*31, 1), dtype=tf.float32), dtype=tf.float32)
...
strings_ = "./logs16cnn_20221028-2217/"
