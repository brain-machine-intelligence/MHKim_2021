...
def load_data_labels(location='dataset_original.mat'): #2.mat'):
    ...
    dir_label_SPE = "Y:/Research/EEG_kdj/EEG_preprocessed_mh/pilot_label_spe"
    dir_label_RPE = "Y:/Research/EEG_kdj/EEG_preprocessed_mh/pilot_label_rpe"
    ...
    np.random.seed(2121)
    shuffle_idx = np.random.permutation(lb_SPE.shape[0])


    return ep[shuffle_idx], lb_SPE[shuffle_idx], lb_RPE[shuffle_idx]
...
for subi in tqdm(range(2)):
    ...
    for sess_i in range(6):
        dir_epoch_ = "Y:/Research/EEG_kdj/EEG_data_pilot/bcr_fil/"
        ...
        if len(lb_tots) == 0:
            ...
            lb_tots[1]=np.concatenate((lb_tots[1],lb_RPE_tot),axis=0)
...
for lbi in [0]: # lbi == # 0 : SPE, 1: RPE
    ...
    for train_ind, test_ind in kf.split(lb_tot):
        ...
        if cv > 0:
            ...
            network.init_net()
            # Y:\Results_BENDR\logs_all\logs_SPE_lr3e7\cv8\label5\model_4000
            network.load_weights("Y:/Results_BENDR/logs_all/logs_SPE_lr3e7/cv8/label5/model_4000/dnn.ckpt")
            # acc = []
            # loss = []
            epoch = []
            summary = {}
