...
class To1020(InstanceTransform):
    ...
    def __call__(self, *x):
        ...
        original_timepoint = x[0].shape[1] # 91000 # 896 = 256 Hz * 3.5 sec

        # case 0: 이상한 방식

        # ch_n = x[0].shape[0]
        # f, t, Sxx = signal.spectrogram(x[0], nperseg=256, window='hamming', fs=256)
        ...
        # freq = f[0:e[0][0]]
        # tfreq = Sxx[:, 0:e[0][0], :]
        # x[0] = torch.from_numpy(np.array(tfreq.reshape(ch_n, -1))) # downstream : (20, 330)
        ...
        # x[0] = torch.from_numpy(tfreq.reshape(-1, len(t)))  # (260, 274) # TODO 221109 (260, 15)
        ...
        return x # 20 by 896, 20 by 15360
