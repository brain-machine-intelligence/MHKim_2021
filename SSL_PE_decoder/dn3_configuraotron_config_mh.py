...
class DatasetConfig:
    ...
    def _load_raw(self, path: Path):
        ...
        if path.suffix in self._extension_handlers:
            ...
            if self.name == 'SPE' or self.name == 'RPE' or self.name == 'mbmf' or self.name == 'behavior' or self.name == 'pmb' or self.name == 'pilot_SPE' or self.name == 'pilot_RPE': # TODO 221114
                self.preload = True # TODO 1207
        ...
        if self._create_raw_recordings:
            ...
            if not isinstance(raw, (mne.Epochs, mne.epochs.EpochsFIF)):  # Annoying other epochs type
                ...
                if self.name == 'SPE' :
                    ...
                    # merging_events
                    self.events = {'0': 0, '1': 1} # 0 : mf, 1: mb
                elif self.name == 'pilot_SPE' or 'pilot_RPE': # TODO 1207
                    if self.hpf is not None or self.lpf is not None:
                        raw = raw.filter(self.hpf, self.lpf)
                    self.events = {'0': 0, '1': 1}
                    # self.events = raw.event_id # 221121
                    # self.events = {'act1': 7, 'act2': 1, 'act3': 8, 'act4': 9, 'act5': 2, 'act6': 3, 'act7': 4, 'act8': 5, 'act9': 11, 'rewa': 6, 'epi1': 10,  'epi2': 12, 'epi3': 13, 'epi4': 14} #{'0': 0, '1': 1}
            ...
            events_from_annotation_, event_dict_ = mne.events_from_annotations(raw) # epochs.events = where, chn, new_id # TODO 221205
            where_a1 = []
            where_a2 = []
