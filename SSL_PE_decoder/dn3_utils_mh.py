...
def merging_events(eegfile, new_id, timing = [1, 2], id = 6, replace_events = False): # TODO : timing = [4, 5] for state time window, timing = [1, 2] for action
    ...
    for pair in zip(where, chn, new_id):
        # print(pair)
        new_for_merge.append(pair) # 154

    label1 = [] # TODO 22105 label balance
    label0 = []
    for event_i in new_for_merge:
        if event_i[2] == 1:
            label1.append(event_i) # 84 # prc28 : 31 # prc37 : 52
        elif event_i[2] == 0 :
            label0.append(event_i) # 70 # prc28 : 28 # prc37 : 43
        # else:
        #     print(event_i[2])

    if len(label1) == len(label0) == 0:
        new_for_merge = []
        for pair in zip([new_for_merge[0][0]], [new_for_merge[0][1]], [0]):
            new_for_merge = [pair]

    elif len(label1) == 0 and len(label0) != 0:  # label0만 유의미
        new_for_merge = []
        for pair in zip([label0[0][0]], [label0[0][1]], [0]):
            new_for_merge = [pair]

    elif len(label0) == 0 and len(label1) != 0:  # label1만 유의미
        new_for_merge = []
        for pair in zip([label1[0][0]], [label1[0][1]], [0]):
            new_for_merge = [pair]
            
    elif len(label0) == len(label1) and len(label0) != 0:  # label 둘다 유의미
        new_for_merge = label0 + label1
        new_for_merge.sort()
            
    elif len(label1) != len(label0):
        minidx = min(len(label1), len(label0))
        label0 = label0[:minidx]
        label1 = label1[:minidx]
        new_for_merge = label0 + label1
        new_for_merge.sort()
    # epoch_data = mne.Epochs(eegfile, new_for_merge)

    return new_for_merge # 140 # prc28 : 56 # prc37 : 86
    # new_for_merge = np.array(new_for_merge)
    # # new_for_merge = new_for_merge.astype(int)
    # return new_for_merge.astype(int)  # event

def merging_events_pilot(epoch_seed, new_ids, event_ids, id = 99, replace_events = False): # epoch_seed, new_ids, event_ids # TODO 221206
    # events_from_annotation, event_dict = mne.events_from_annotations(eegfile)

    chn = [0 for _ in range(len(epoch_seed))]  # np.zeros(len(where))
    new_for_merge = []
    for pair in zip(epoch_seed, chn, new_ids):
        # print(pair)
...
def make_epochs_from_raw(raw: mne.io.Raw, tmin, tlen, event_ids=None, baseline=None, decim=1, filter_bp=None,
    ...
    try:
        if use_annotations and name == 'SPE':
            ...
            #     subj_i) + '/' + sess_i + '_spe_label.mat'  # SPE_label.mat'
            filepath = 'Y:/Research/EEG_kdj/EEG_preprocessed_mh/prc28_spe_per_subj/subj_' + str( # TODO 221205
                subj_i) + '/' + sess_i + '_spe_label.mat'  # SPE_label.mat'
            # filepath = 'Y:/Research/EEG_kdj/EEG_preprocessed_mh/prc37_spe_per_subj/subj_' + str(
            #     subj_i) + '/' + sess_i + '_spe_label.mat'  # SPE_label.mat'
            # filepath = 'Y:/Research/EEG_kdj/EEG_preprocessed_mh/prc30_spe_per_subj/subj_' + str(
            #     subj_i) + '/' + sess_i + '_spe_label.mat'  # SPE_label.mat' # TODO 221124
            # filepath = 'Y:/Research/EEG_kdj/EEG_preprocessed_mh/prc70_spe_per_subj/subj_' + str(
            #     subj_i) + '/' + sess_i + '_spe_label.mat'  # SPE_label.mat'

            data_mat = scipy.io.loadmat(filepath)
            new_ids = data_mat.get('spe_label')[0] # (1, 164)
            ...
            #     subj_i) + '/' + sess_i + '_rpe_label.mat'  # RPE_label.mat'
            filepath = 'Y:/Research/EEG_kdj/EEG_preprocessed_mh/prc28_rpe_per_subj/subj_' + str( # TODO 221205
                subj_i) + '/' + sess_i + '_rpe_label.mat'  # RPE_label.mat'
            # filepath = 'Y:/Research/EEG_kdj/EEG_preprocessed_mh/prc37_rpe_per_subj/subj_' + str(
            #     subj_i) + '/' + sess_i + '_rpe_label.mat'  # RPE_label.mat'
            # filepath = 'Y:/Research/EEG_kdj/EEG_preprocessed_mh/prc30_rpe_per_subj/subj_' + str(
            #     subj_i) + '/' + sess_i + '_rpe_label.mat'  # RPE_label.mat' # TODO 221124
            # filepath = 'Y:/Research/EEG_kdj/EEG_preprocessed_mh/prc70_rpe_per_subj/subj_' + str(
            #     subj_i) + '/' + sess_i + '_rpe_label.mat'  # RPE_label.mat'
        ...
        elif name == 'pilot_SPE': # TODO 221206
            # events = mne.find_events(raw)
            # raw.filenames == Y:\\BENDR_for_pretraining\\pilot_2\\subj01\\sub01_sess1\\220523_hj_1_20220523_122723.mff\\signal1.bin'

            sess_i = raw.filenames[0].split('\\')[-3][6:11]  # sess1
            subj_i = raw.filenames[0].split('\\')[-3][3:5]  # subj001 -> 01
            # labeling 만 잘 하면 될듯...! # TODO
            name_file = raw.filenames[0].split('\\')[-2][:-4]
            # epoch_seed 읽기 Y:\Research\EEG_kdj\EEG_data_pilot\epoch_seed
            # epoch_seed_path = "Y:/Research/EEG_kdj/EEG_data_pilot/epoch_seed"
            # epoch_seed_ = scipy.io.loadmat(epoch_seed_path + "/epoch_seed_" + name_file + "_bcr.mat")
            epoch_seed_path = "Y:/Research/EEG_kdj/EEG_preprocessed_mh/pilot_after_balance/pilot_epoch_seed/SPE"
            epoch_seed_ = scipy.io.loadmat(epoch_seed_path + "/subj_0{0}/{1}_epoch_seed.mat".format(subj_i, sess_i))

            epoch_seed = epoch_seed_['epoch_seed_sess'].squeeze() # (1, 229) -> (229,)

            # Y:\Research\EEG_kdj\EEG_preprocessed_mh\pilot_label_spe
            # filepath = 'Y:/Research/EEG_kdj/EEG_preprocessed_mh/pilot_label_spe/subj_0' + str(
            #     subj_i) + '/' + sess_i + '_spe_label.mat'
            filepath = 'Y:/Research/EEG_kdj/EEG_preprocessed_mh/pilot_after_balance/pilot_label/SPE/subj_0' + str(
                subj_i) + '/' + sess_i + '_spe_label.mat'

            data_mat = scipy.io.loadmat(filepath)
            # name_ff = 'final_label_SPE_{0}_{1}'.format(subj_i[1], sess_i[-1])
            name_ff = 'list_label_for_save_sess'
            new_ids = data_mat.get(name_ff)[0]
            events = merging_events_pilot(epoch_seed, new_ids, event_ids)
            # return mne.Epochs(raw, events, tmin=tmin, tmax=tmin + tlen - 1 / sfreq, preload=True, decim=decim,
            #                   baseline=baseline, reject_by_annotation=drop_bad, event_repeated='merge')

        elif name == 'pilot_RPE': # TODO 221206
            # events = mne.find_events(raw)
            # raw.filenames == Y:\\BENDR_for_pretraining\\pilot_2\\subj01\\sub01_sess1\\220523_hj_1_20220523_122723.mff\\signal1.bin'
            ...
            # epoch_seed_path = "Y:/Research/EEG_kdj/EEG_data_pilot/epoch_seed"
            # epoch_seed_ = scipy.io.loadmat(epoch_seed_path + "/epoch_seed_" + name_file + "_bcr.mat")
            epoch_seed_path = "Y:/Research/EEG_kdj/EEG_preprocessed_mh/pilot_after_balance/pilot_epoch_seed/RPE"
            epoch_seed_ = scipy.io.loadmat(epoch_seed_path + "/subj_0{0}/{1}_epoch_seed.mat".format(subj_i, sess_i))

            epoch_seed = epoch_seed_['epoch_seed_sess'].squeeze()  # (1, 229) -> (229,)

            # Y:\Research\EEG_kdj\EEG_preprocessed_mh\pilot_label_rpe
            # filepath = 'Y:/Research/EEG_kdj/EEG_preprocessed_mh/pilot_label_rpe/subj_0' + str(
            #     subj_i) + '/' + sess_i + '_rpe_label.mat'
            filepath = 'Y:/Research/EEG_kdj/EEG_preprocessed_mh/pilot_after_balance/pilot_label/RPE/subj_0' + str(
                subj_i) + '/' + sess_i + '_rpe_label.mat'

            data_mat = scipy.io.loadmat(filepath)
            # name_ff = 'final_label_RPE_{0}_{1}'.format(subj_i[1], sess_i[-1])
            name_ff = 'list_label_for_save_sess'
            new_ids = data_mat.get(name_ff)[0]
            events = merging_events_pilot(epoch_seed, new_ids, event_ids)
            # return mne.Epochs(raw, events, tmin=tmin, tmax=tmin + tlen - 1 / sfreq, preload=True, decim=decim,
    ...
    return mne.Epochs(raw, events, tmin=tmin, tmax=tmin + tlen - 1 / sfreq, preload=True, decim=decim,
                      baseline=baseline, reject_by_annotation=drop_bad, event_repeated= 'error') # TODO drop...when realistic 원래는 'error'
