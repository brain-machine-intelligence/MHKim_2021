...
def get_ds(name, ds):
    ...
    return dataset # thinker로 이루어져있는 datasets
...
#
# def get_iterator_pilot(name, ds):
#     dataset = get_ds(name, ds) # channel 20..
#     try:
#         ds.train_params.pop('_d')
#     except:
#         pass
#     specific_test = ds.test_subjects if hasattr(ds, 'test_subjects') else None
#     test_person_id = [t for t in dataset.get_thinkers()]  # here ['subj01', 'subj02']
#     training = [[v] for v in test_person_id]
#     training = dataset._make_like_me(training)
#     iterator = []
