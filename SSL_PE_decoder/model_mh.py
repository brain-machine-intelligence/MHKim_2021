...
class Classifier(DN3BaseModel):
    ...
    def make_new_classification_layer(self):
        ...
        """
        # 3개 버전 
        self.flat = Flatten() # 512 128 32 2
        self.classifier_1 = nn.Linear(self.num_features_for_classification, 128) # 512, 2, bias=True # TODO 221203
        nn.init.xavier_normal_(self.classifier_1.weight)
        self.classifier_1.bias.data.zero_()

        self.classifier_2 = nn.Linear(128, 32)  # 512, 2, bias=True # TODO 221203
        nn.init.xavier_normal_(self.classifier_2.weight)
        self.classifier_2.bias.data.zero_()

        self.classifier_3 = nn.Linear(32, self.targets)  # 512, 2, bias=True # TODO 221203
        nn.init.xavier_normal_(self.classifier_3.weight)
        self.classifier_3.bias.data.zero_()

        self.classifier = nn.Sequential(self.flat, self.classifier_1, nn.LayerNorm(128), nn.ReLU(),
                                        self.classifier_2, nn.LayerNorm(32), nn.ReLU(), self.classifier_3)
        
        
        # 2개 버전
        self.flat = Flatten()  # 512  128 2
        self.classifier_1 = nn.Linear(self.num_features_for_classification, 128)  # 512, 2, bias=True # TODO 221203
        nn.init.xavier_normal_(self.classifier_1.weight)
        self.classifier_1.bias.data.zero_()

        self.classifier_2 = nn.Linear(128, self.targets)  # 512, 2, bias=True # TODO 221203
        nn.init.xavier_normal_(self.classifier_2.weight)
        self.classifier_2.bias.data.zero_()

        self.classifier = nn.Sequential(self.flat, self.classifier_1, nn.LayerNorm(128), nn.ReLU(), self.classifier_2)
        ...
        # classifier = nn.Linear(self.num_features_for_classification, self.targets)  # 2048, 2, bias=True
        # nn.init.xavier_normal_(classifier.weight)
        # classifier.bias.data.zero_()
        # self.classifier = nn.Sequential(Flatten(), classifier)  # TODO 221130
    ...
    def classifier_forward(self, features):
        ...
        # x = F.relu(x)
        # return self.classifier_post(x)
        return self.classifier(features) # downstream : (60, 512) -> (60, 2) # TODO 221130
...
class BENDRClassifier(Classifier): # nn.Module
    ...
    def __init__(self, targets, samples, channels,
                 ...
                 encoder_h=256, # original sampling rate # TODO 221130 256
                 encoder_w=(3, 2), #, 2), #, 2, 2, 2), # TODO 221123 # TODO 221130
                 encoder_do=0.15, # drop #  encoder_do=0. # 1201
                 projection_head=False,
                 encoder_stride=(3, 2), #, 2), #, 2, 2, 2), # TODO 221123 # TODO 221130
                 hidden_feedforward=2560, # TODO 221130 3076
                 heads=8,
                 context_layers=3,# TODO 221130 # 8
                 context_do=0.2,# 1201 context_do=0.15
                 activation='gelu',
                 position_encoder=25,
                 layer_drop=0.1,# 원래 layer_drop=0.# 1201
                 mask_p_t=0.1,
                 mask_p_c=0.004,
                 mask_t_span=6,
                 mask_c_span=64,
        ...
        self.encoder = ConvEncoderBENDR(in_features=channels, encoder_h=encoder_h, enc_width=encoder_w,# todo 들어가나..?
                                        dropout=encoder_do, projection_head=projection_head,
                                        enc_downsample=encoder_stride)
        self.contextualizer = BENDRContextualizer(encoder_h, hidden_feedforward=hidden_feedforward, heads=heads, # Transformer # todo 들어가나..?
