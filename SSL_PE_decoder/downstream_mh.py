...
if __name__ == '__main__':
    ...
    for ds_name, ds in tqdm.tqdm(experiment.datasets.items(), total=len(experiment.datasets.items()), desc='Datasets'):
        # added_metrics, retain_best, _ = utils_mh.get_ds_added_metrics(ds_name, args.metrics_config) # TODO 221202
        for fold, (training, validation, test) in enumerate(tqdm.tqdm(utils_mh.get_lmoso_iterator(ds_name, ds))):
            ...
            # writer = SummaryWriter(log_dir= "./runs_2") # TODO 221206
            writer = True # TODO 221208
            flg = False
            ...
            model.to(device)
            # process = StandardClassification(model, metrics=added_metrics, writer=writer)
            process = StandardClassification(model, writer = writer) # TODO 221202 221208
            # process.set_optimizer(torch.optim.Adam(process.parameters(), ds.lr))
            ...
            def simple_checkpoint(metrics):  # ds_name, fold, args.model
                ...
                path = 'checkpoints/{0}/{1}_{2}/{3}_fold'.format(ds_name, args.model, "loss", fold + 1) # TODO 221202
                if not os.path.exists(path):
                    os.makedirs(path)
            ...
            #             retain_best=retain_best, pin_memory=False, **ds.train_params)
            a, b = process.fit(training_dataset=training, validation_dataset=validation,
                               log_callback=simple_checkpoint, warmup_frac=0.1, pin_memory=False, fold = fold +1, **ds.train_params) # TODO 221202 221208
            print(a)
            print("--")
            print(b)
            if args.results_filename:
                ...
                file_name = './results/' + time_now + '_' + parser.parse_args().model + '_{}.xlsx'.format("loss") # TODO 221202
