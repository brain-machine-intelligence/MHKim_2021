...
import random
from torch.utils.tensorboard import SummaryWriter # TODO 221208
...
class BaseProcess(object):
    ...
    def __init__(self, lr=0.001, metrics=None, evaluation_only_metrics=None, l2_weight_decay=0.01, cuda=None, **kwargs): # TODO: l2_weight_decay = 0.01 이었음 0.00001
        """
        ...
        """
        if cuda is None: # l2_weight_decay CHECK
    ...
    def _get_batch(self, iterator):
        ...
        # f, t, Sxx = signal.spectrogram(batch[0][0, :, :].cpu(), nperseg=256, window='hamming', fs=256)
        # e = np.where(f >= 55)
        # freq = f[0:e[0][0]]
        # tfreq = Sxx[:, 0:e[0][0], :]
        # import matplotlib.pyplot as plt
        # fig, axs = plt.subplots(5,4)
        # axs[0, 0].pcolormesh(t, freq, tfreq[0, :, :], shading='gouraud')
        # axs[1, 0].pcolormesh(t, freq, tfreq[1, :, :], shading='gouraud')
        # axs[2, 0].pcolormesh(t, freq, tfreq[2, :, :], shading='gouraud')
        # axs[3, 0].pcolormesh(t, freq, tfreq[3, :, :], shading='gouraud')
        # axs[4, 0].pcolormesh(t, freq, tfreq[4, :, :], shading='gouraud')
        # axs[0, 1].pcolormesh(t, freq, tfreq[5, :, :], shading='gouraud')
        # axs[1, 1].pcolormesh(t, freq, tfreq[6, :, :], shading='gouraud')
        # axs[2, 1].pcolormesh(t, freq, tfreq[7, :, :], shading='gouraud')
        # axs[3, 1].pcolormesh(t, freq, tfreq[8, :, :], shading='gouraud')
        # axs[4, 1].pcolormesh(t, freq, tfreq[9, :, :], shading='gouraud')
        # axs[0, 2].pcolormesh(t, freq, tfreq[10, :, :], shading='gouraud')
        # axs[1, 2].pcolormesh(t, freq, tfreq[11, :, :], shading='gouraud')
        # axs[2, 2].pcolormesh(t, freq, tfreq[12, :, :], shading='gouraud')
        # axs[3, 2].pcolormesh(t, freq, tfreq[13, :, :], shading='gouraud')
        # axs[4, 2].pcolormesh(t, freq, tfreq[14, :, :], shading='gouraud')
        # axs[0, 3].pcolormesh(t, freq, tfreq[15, :, :], shading='gouraud')
        # axs[1, 3].pcolormesh(t, freq, tfreq[16, :, :], shading='gouraud')
        # axs[2, 3].pcolormesh(t, freq, tfreq[17, :, :], shading='gouraud')
        # axs[3, 3].pcolormesh(t, freq, tfreq[18, :, :], shading='gouraud')
        # axs[4, 3].pcolormesh(t, freq, tfreq[19, :, :], shading='gouraud')
        # # plt.pcolormesh(t, freq, tfreq[0, :, :], shading='gouraud')
    ...
    def calculate_loss(self, inputs, outputs):
        ...
        l_temp_ = loss_fn(outputs, inputs[-1])

        if torch.any(torch.isnan(l_temp_)):
        ...
        l_temp = l_temp_ + l2_lambda * l2_norm
    ...
    def train_step(self, *inputs) : #, epoch): # TODO 221021
        ...
        # self._trainables = ['encoder', 'loss_fn', 'context_fn']

        loss = self.calculate_loss(inputs, outputs)  # 0.7084 # inputs = 2 by 60 by 20 by 896, outputs = 60 by 2

        # loss_ = self.calculate_loss(inputs, outputs) # 0.7084 # inputs = 2 by 60 by 20 by 896, outputs = 60 by 2
        # l2_lambda = 0.001
        # l2_norm = sum(p.pow(2.0).sum() for p in self.parameters())
        # loss = loss_ + l2_lambda * l2_norm # 2.228
    ...
    def fit(self, training_dataset, epochs=1, validation_dataset=None, step_callback=None,
            ...
            validation_interval=None, train_log_interval=None, fold = None, **loader_kwargs): # TODO 221208
        """
        ...
        if self.writer:# TODO 221208
            writer = SummaryWriter("./runs_lr1e6_cv_".format(fold))
        ...
        for epoch in epoch_bar:
            ...
            for name_i in metric_name_for_writer:
                # self.writer.add_scalar(name_i + "_train", metrics[name_i], self.epoch)
                writer.add_scalar(name_i + "_train", metrics[name_i], self.epoch)# TODO 221208
            ...
            if validation_dataset is not None:
                ...
                for name_i in metric_name_for_writer:
                    ...
                    writer.add_scalar(name_i + "_val", metrics[name_i],
                                      self.epoch)  # TODO 221208
...
class StandardClassification(BaseProcess):
    ...
    def __init__(self, classifier: torch.nn.Module, loss_fn=None, cuda=None, metrics=None, learning_rate=0.01,
        ...
        super(StandardClassification, self).__init__(cuda=cuda, lr=learning_rate, classifier=classifier,loss_fn=torch.nn.CrossEntropyLoss(), # TODO 221121
    ...
    def fit(self, training_dataset, epochs=1, validation_dataset=None, step_callback=None, epoch_callback=None,
            batch_size=8, warmup_frac=0.2, retain_best='loss', balance_method=None, fold = None, **loader_kwargs):# TODO 221208
        """
        sklearn/keras-like convenience method to simply proceed with training across multiple epochs of the provided
        dataset
        ...
        return super(StandardClassification, self).fit(training_dataset, epochs=epochs, step_callback=step_callback,
                                                       ...
                                                       balance_method=balance_method,fold = fold, # TODO 221208
                                                       **loader_kwargs)
