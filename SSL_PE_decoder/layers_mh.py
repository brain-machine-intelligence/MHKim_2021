...
class ConvEncoderBENDR(_BENDREncoder):
    ...
    # def __init__(self, in_features, encoder_h=256, enc_width=(3, 2, 2), # TODO 221123
    #              dropout=0., projection_head=False, enc_downsample=(3, 2, 2)): # TODO 221110 # TODO 221123
    def __init__(self, in_features, encoder_h=256, enc_width=(3, 2),  # TODO 221130
                 dropout=0.2, projection_head=False, enc_downsample=(3, 2)):
        ...
        # in_features = 260 # TODO 221109 221115 case1
        # in_featurses = 340  # TODO 221117 3 case - freq dir
        # in_features = 20 # TODO 221117 : 3 case - downstream case
...
class ConvEncoder_DJ(_BENDREncoder):
    ...
    def __init__(self, in_features, encoder_h=256, enc_width=(3, 2),  # TODO 221130
        ...
        self.encoder = nn.Sequential()
        self.encoder.add_module("Encoder_{1}", nn.Sequential(nn.Conv2d(16, 32, 5, stride=2, padding='same')))

        self.conv1 = nn.Conv2d(16, 32, 5, stride=2, padding='same')
        self.conv1.apply(self.init_bert_params_mh)
        self.batchnorm1 = nn.BatchNorm2d

        # for i, (width, downsample) in enumerate(zip(enc_width, enc_downsample)):
        #     self.encoder.add_module("Encoder_{}".format(i), nn.Sequential(
        #         nn.Conv2d(in_features, encoder_h, width, stride=downsample, padding=width // 2), # in_channel, out_channel, kernel_size, stride, dilation = 16, 32, 5, stride = [1,2,2,1], padding='same'
        #         nn.Dropout2d(dropout),
        #         nn.GroupNorm(encoder_h // 2, encoder_h),
        #         nn.GELU(),
        #     )) # layer_name, layer
    ...
    def init_bert_params_mh(self, module):
...
class BENDRContextualizer(nn.Module): # Transformer 전처리 + transformer
    ...
    def __init__(self, in_features, hidden_feedforward=3076, heads=8, layers=3, dropout=0.15, activation='gelu',
        ...
        layers = 3 #2 #4 # TODO 221108 221123 # TODO 221130
    ...
    def forward(self, x, mask_t=None, mask_c=None):
        ...
        bs, feat, seq = x.shape # batch_size, n_features, n_seqs || downstream : (60, 512, 13) 원래는 75
        self.mask_t_span = int(0.1 * seq)  # TODO 221122
        self.mask_c_span = int(0.1 * feat)
        if self.training and self.finetuning: # pretrain :no, downstream : yes
            if mask_t is None and self.p_t > 0: #yes
                mask_t = _make_mask((bs, seq), self.p_t, x.shape[-1], self.mask_t_span) # yes zero OR True # 7 # (60, 28)
            if mask_c is None and self.p_c > 0: #yes
                mask_c = _make_mask((bs, feat), self.p_c, x.shape[1], self.mask_c_span) # yes zero OR True # 51 # (60, 512)

        # Multi-gpu workaround, wastes memory
        x = x.clone() # (60, 512, 1) # (2, 512, 2560)

        if mask_t is not None:
            x.transpose(2, 1)[mask_t] = self.mask_replacement # 512
