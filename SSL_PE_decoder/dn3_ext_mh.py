...
from layers_mh import _BENDREncoder, ConvEncoderBENDR, ConvEncoder_DJ, _Hax, BENDRContextualizer, _make_span_from_seeds, _make_mask, Flatten, Permute
...
class BENDRClassification(Classifier):
    ...
    def features_forward(self, *x):
        ...
        # [60 by 20 by 896] -> [60 by 512 by 150]
        ...
        context = self.contextualizer(encoded) # 60 by 512 by 151
    ...
    def __init__(self, targets, samples, channels, encoder_h=512, contextualizer_hidden=2560, projection_head=False, # TODO 221130 contextualizer_hidden=3076
                 new_projection_layers=0, dropout=0.15, trial_embeddings=None, layer_drop=0.1, keep_layers=None,
                 mask_p_t=0.01, mask_p_c=0.005, mask_t_span=0.1, mask_c_span=0.1, multi_gpu=False): # TODO 원래 dropout=0. , layer_drop=0 1201
        self.encoder_h = encoder_h
...
class DJ_Classification(Classifier):
    ...
    def __init__(self, targets, samples, channels, encoder_h=512, contextualizer_hidden=2560, projection_head=False, # TODO 221130 contextualizer_hidden=3076
        ...
        encoder = ConvEncoder_DJ(channels, encoder_h=encoder_h, dropout=dropout, projection_head=projection_head) # TODO channels = 20
        encoded_samples = encoder.downsampling_factor(samples)
...
class BendingCollegeWav2Vec(BaseProcess):
    ...
    def forward(self, *inputs):
        z = self.encoder(inputs[0]) # (2, 20, 15360) -> (2, 512, 2560) : encoder == (3, 2)

        if self.permuted_encodings: #no
            z = z.permute([1, 2, 0])

        unmasked_z = z.clone() # (10, 512, 23) # (8, 512, 2560)

        batch_size, feat, samples = z.shape # (8, 512, 2560)
        ...
        if self._training:
            ...
            mask[:, _make_span_from_seeds((samples // half_avg_num_seeds) * np.arange(half_avg_num_seeds).astype(int),
                                              self.mask_span)] = True
        # mask : (10, 2)
        c = self.context_fn(z, mask) # (10, 512, 24) : token 넣은 거.. # (2, 512, 2561)
